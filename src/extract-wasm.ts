/**
 * tree-sitter WASM extraction adapter (ADR-0001): Python, Go, Rust, Java.
...
let parserPromise: Promise<{ parser: { parse: (s: string) => unknown; setLanguage: (l: unknown) => void }; load: (lang: string) => Promise<unknown> }> | undefined
...
async function getParser(): Promise<{ parser: { parse: (s: string) => unknown; setLanguage: (l: unknown) => void }; load: (lang: string) => Promise<unknown> }> {
  if (parserPromise === undefined) {
    parserPromise = (async () => {
      ...
      const load = (lang: string): Promise<unknown> => {
        let cached = languageCache.get(lang)
        if (cached === undefined) {
          // lang is the npm package (tree-sitter-python); its wasm ships
          // as tree-sitter-<grammar>.wasm inside the package root.
          const pkgRoot = nodeRequire.resolve(`${lang}/package.json`)
          const grammar = lang.replace(/^tree-sitter-/, '')
          const wasmPath = join(pkgRoot, '..', `tree-sitter-${grammar}.wasm`)
...
/** Language registry: extension → npm grammar package. */
const WASM_LANGUAGES: Record<string, { pkg: string; ext: string }> = {
...
export async function relationsForWasmFile(
  ...
  const language = await load(langInfo.pkg)
